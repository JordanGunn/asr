...
- `--agent-flags "FLAGS"` — Additional agent CLI flags (space-separated; appended to defaults)
...
- **Codex**: adds `--skip-git-repo-check` + `--dangerously-bypass-approvals-and-sandbox` by default.
- **Copilot**: adds `--yolo --allow-all-tools --allow-all-paths --allow-all-urls` by default.
- **Claude**: adds `--dangerously-skip-permissions` by default.
- **OpenCode**: no defaults yet; use `--agent-flags` if needed.

**Trusted directory guidance:**

- **Codex**: Use `-C <dir>` or `--add-dir` to scope work if you want a narrower root.
- **Copilot**: Use `--add-dir` to constrain to specific directories if needed.
- **Claude**: Use `--add-dir` or configure allowed tools/permissions in Claude settings.
- **OpenCode**: Use `permission.external_directory` in `opencode.json` or `--agent-flags` if supported.

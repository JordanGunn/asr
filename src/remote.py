...
def _get_auth_token(platform: str) -> Optional[str]:
...
def fetch_gitlab_directory(
    ...
    try:
        ...
        if e.code == 404:
            ...
            raise RemoteRateLimitError(
                "GitLab API rate limit exceeded. Set GITLAB_TOKEN environment variable."
            )
        else:
            raise RemoteNetworkError(f"HTTP error {e.code}: {e.reason}")
    except urlerror.URLError as e:
...
def _download_file(url: str, dest: Path, token: Optional[str] = None, platform: str = "github"):
...
def _fetch_gitlab_recursive(
    ...
    for entry in entries:
        ...
        if entry_type == "blob":  # File
            ...
            download_url = f"https://gitlab.com/api/v4/projects/{project_id}/repository/files/{file_path_encoded}/raw?ref={ref}"
